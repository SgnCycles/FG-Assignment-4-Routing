...
const Continent = ({ coordinates, updateCoordinates }) => {
  ...
  const countriesData = useFetch("/countries/v5");
  const continentData = continents.find(
    (continent) =>
      continent.name.toLowerCase() === normalizedContinentName.toLowerCase(),
  );
  ...
  const filteredContinentCountries = countriesData.filter((country) =>
    country.continents.includes(continentData.name),
  );

  return (
    <>
      <Globe coordinates={coordinates} />
      <h1 className={styles.continentHeader}>
        Welcome to {name.charAt(0).toUpperCase() + name.slice(1)}
      </h1>
      <div className={styles.countryContainer}>
        {filteredContinentCountries &&
          filteredContinentCountries.map((country, index) => (
